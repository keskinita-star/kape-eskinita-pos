...
import { getIngredients, addIngredient, updateIngredient, deleteIngredient } from "../../services/ingredientService";
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <h2 style={{ fontSize:20, fontWeight:700, color:"#1a1814", letterSpacing:"-0.5px" }}>Ingredient Inventory</h2>
          <p style={{ fontSize:13, color:"#9a9690", marginTop:2 }}>{ingredients.length} ingredients tracked</p>
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {outItems.map(i => (
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", fontSize:12 }}>
                <span style={{ color:"#1a1814", fontWeight:500 }}>{i.name}</span>
            ...
            {lowItems.map(i => (
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", fontSize:12 }}>
                <span style={{ color:"#1a1814", fontWeight:500 }}>{i.name}</span>
                <span style={{ color:"#b45309", fontWeight:600 }}>Only {i.stock} {i.unit} left (min: {i.minStock})</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background:"#fff", border:"1px solid #e8e2d9", borderRadius:16, overflow:"hidden" }}>
          <div style={{ padding:"16px 24px", borderBottom:"1px solid #f0ede8", background:"#fdfcfb" }}>
            <div style={{ fontSize:15, fontWeight:700, color:"#1a1814" }}>{editing ? "Edit Ingredient" : "New Ingredient"}</div>
          </div>
          <div style={{ padding:24, display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14 }}>
            <div style={{ gridColumn:"1/-1" }}>
              <label style={lbl}>Ingredient Name</label>
              <input style={inp} value={form.name} onChange={e => setForm(f=>({...f,name:e.target.value}))} placeholder="e.g. Arabica Coffee Beans" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input style={inp} value={form.supplier} onChange={e => setForm(f=>({...f,supplier:e.target.value}))} placeholder="e.g. Benguet Coffee" />
            </div>
            <div>
              <label style={lbl}>Current Stock</label>
              <input type="number" min="0" style={inp} value={form.stock} onChange={e => setForm(f=>({...f,stock:e.target.value}))} placeholder="0" required />
            </div>
            <div>
              <label style={lbl}>Min Stock (alert threshold)</label>
              <input type="number" min="0" style={inp} value={form.minStock} onChange={e => setForm(f=>({...f,minStock:e.target.value}))} placeholder="0" required />
            </div>
            <div>
              <label style={lbl}>Cost per Unit (₱)</label>
              <input type="number" min="0" step="0.01" style={inp} value={form.costPerUnit} onChange={e => setForm(f=>({...f,costPerUnit:e.target.value}))} placeholder="0.00" required />
            </div>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
        {["All","Low Stock","Out of Stock",...CATEGORIES].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            {f}
            {f==="Low Stock" && lowItems.length > 0 && <span style={{ marginLeft:5, background:"#b45309", color:"#fff", borderRadius:10, padding:"1px 6px", fontSize:10 }}>{lowItems.length}</span>}
            {f==="Out of Stock" && outItems.length > 0 && <span style={{ marginLeft:5, background:"#dc2626", color:"#fff", borderRadius:10, padding:"1px 6px", fontSize:10 }}>{outItems.length}</span>}
          </button>
        ))}
      </div>
      ...
      {loading ? <p style={{ fontSize:13, color:"#9a9690" }}>Loading...</p> :
        filtered.length === 0 ? (
          <div style={{ background:"#fff", border:"1px solid #e8e2d9", borderRadius:12, padding:40, textAlign:"center" }}>
            <div style={{ fontSize:32, marginBottom:8 }}>📦</div>
            <p style={{ fontSize:13, color:"#9a9690" }}>No ingredients found. Add your first ingredient above.</p>
          </div>
        ) : (
            ...
            <thead>
              <tr style={{ background:"#f5f3ee" }}>
                {["Ingredient","Category","Stock","Min Stock","Cost/Unit","Supplier","Actions"].map(h => (
                  <th key={h} style={{ padding:"10px 14px", textAlign:"left", fontSize:11, fontWeight:600, color:"#9a9690", borderBottom:"1px solid #e8e2d9", textTransform:"uppercase", letterSpacing:"0.5px", whiteSpace:"nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(item => {
                ...
                return (
                  <tr key={item.id} style={{ borderBottom:"1px solid #f5f3ee" }}>
                    <td style={{ padding:"12px 14px", fontWeight:600, color:"#1a1814" }}>{item.name}</td>
                    <td style={{ padding:"12px 14px" }}>
                      <span style={{ fontSize:11, padding:"2px 8px", borderRadius:20, background:"#f5f3ee", color:"#6b6860" }}>{item.category}</span>
                    </td>
                    <td style={{ padding:"12px 14px" }}>
                      {restockId === item.id ? (
                        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                          <input type="number" min="1" value={restockQty} onChange={e => setRestockQty(e.target.value)}
                            ...
                            style={{ width:70, padding:"5px 8px", border:"1px solid #e8e2d9", borderRadius:6, fontSize:12 }} />
                          <button onClick={() => handleRestock(item.id)}
                            style={{ padding:"4px 8px", borderRadius:6, background:"#166534", color:"#fff", border:"none", fontSize:11, cursor:"pointer", fontWeight:600 }}>+Add</button>
                          <button onClick={() => { setRestockId(null); setRestockQty(""); }}
                            style={{ padding:"4px 6px", borderRadius:6, background:"none", border:"1px solid #e8e2d9", fontSize:11, cursor:"pointer" }}>✕</button>
                        </div>
                      ) : (
                        <span style={{ fontSize:12, padding:"3px 10px", borderRadius:20, fontWeight:500, background:status.bg, color:status.color }}>
                          {status.label}
                        </span>
                      )}
                    </td>
                    <td style={{ padding:"12px 14px", color:"#9a9690", fontSize:12 }}>{item.minStock} {item.unit}</td>
                    <td style={{ padding:"12px 14px", color:"#1a1814", fontWeight:500 }}>{formatCurrency(item.costPerUnit)}/{item.unit}</td>
                    <td style={{ padding:"12px 14px", color:"#6b6860" }}>{item.supplier || "—"}</td>
                    <td style={{ padding:"12px 14px" }}>
                      <div style={{ display:"flex", gap:6 }}>
                        <button onClick={() => { setRestockId(item.id); setRestockQty(""); }}
                          style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #bbf7d0", background:"#f0fdf4", fontSize:11, color:"#166534", cursor:"pointer", fontWeight:600 }}>
                          Restock
                        </button>
                        <button onClick={() => handleEdit(item)}
                          style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #e8e2d9", background:"none", fontSize:11, color:"#1a1814", cursor:"pointer" }}>
                          Edit
                        </button>
                        <button onClick={() => handleDelete(item.id)}
                          style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #fca5a5", background:"none", fontSize:11, color:"#dc2626", cursor:"pointer" }}>
                          Delete
                        </button>
                      </div>
                    </td>
